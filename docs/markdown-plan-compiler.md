...
### Epic Level
- Must be top level and start with the token `# Epic: `
...
### Task Level
- Each task is a level-two heading starting with `## Task <number>: `
- Tasks can have metadata fields as the first lines after the heading
- Metadata fields are marked with `**field: **` syntax

### Subtask Level
- Subtasks are level-three headings under Tasks
...
1. **Predecessors**: `**Predecessors:** Task <nr1>, Task <nr2>, ...`
...
- `MetadataPredecessors { task_numbers: Vec<u32> }`
...
- `Epic { title: String, content: Vec<ContentBlock>, tasks: Vec<Task> }`
...
Validate that all task numbers referenced in `Predecessors` fields exist in the plan using HashMap lookups.
