...
- **`rhei run` asks the whole completion condition before a pass skips an agent
  ...
  [§FS-rhei-run.3](../functional-spec/rhei-run.spec.md#3-execution-loop) step 5 already said that no transition fires and the ticket
  ...
  ([§FS-rhei-agents.8.4](../functional-spec/rhei-agents.spec.md#84-spawn-records)), and each attempt keeps its own transcript rather than
  ...
  it is and the run names what it owes ([§FS-rhei-agents.3.2.3](../functional-spec/rhei-agents.spec.md#323-attempt-budget)). `visits:` bounds
  ...
  together. [§FS-rhei-agents.3.2](../functional-spec/rhei-agents.spec.md#32-completion-condition) [§FS-rhei-states.3.3](../functional-spec/rhei-states.spec.md#33-terminal-result) (PR #106)
...
- **`rhei list --ready` is the ready set, not a second opinion about it.** It
  ...
  [§FS-rhei-list.3.1](../functional-spec/rhei-list.spec.md#31-what---ready-means) already promised the listing "lists exactly the set `rhei
  ...
  [§FS-rhei-next.3](../functional-spec/rhei-next.spec.md#3-default-behavior-claim-mode) [§FS-rhei-run.3](../functional-spec/rhei-run.spec.md#3-execution-loop) (PR #104)
...
- `rhei run` looks for a **member rhei's required `inputs:` under the rhei that
  ...
  [§AR-rhei-panta.5](../architecture/rhei-panta.spec.md#5-execution-root-and-per-rhei-runtime) [§FS-rhei-run-report.3.1](../functional-spec/rhei-run-report.spec.md#31-layout) (PR #101)
...
- **`rhei reset` no longer erases a pre-authored chain.** It returned every
  ...
  [§FS-rhei-supervision.7](../functional-spec/rhei-supervision.spec.md#7-example) documents and the `supervised-delivery` template ships
  ...
  brief their own state gates on — the release gate of [§FS-rhei-supervision.5.2](../functional-spec/rhei-supervision.spec.md#52-the-brief)
  ...
  ([§FS-rhei-states.9.2](../functional-spec/rhei-states.spec.md#92-resolution)) and those children share both. Reset now returns each
  ...
  destructive. [§FS-rhei-reset.2.2](../functional-spec/rhei-reset.spec.md#22-authored-state) [§FS-rhei-reset.4](../functional-spec/rhei-reset.spec.md#4-output) (PR #102)
...
- A new grund kind, **`REQ`** (`docs/requirements/`), for cross-cutting
  ...
  cross-platform rule is the first: [§REQ-cross-platform](../requirements/cross-platform.md#req-cross-platform-one-tool-on-linux-macos-and-windows). (PR #98)
...
- Rhei is a **cross-platform tool by requirement**, not by accident: Linux,
  ...
  [§GOAL-rhei-outcomes](../functional-spec/goals.md#goal-rhei-outcomes-goals) [§REQ-cross-platform](../requirements/cross-platform.md#req-cross-platform-one-tool-on-linux-macos-and-windows) (PR #96)
...
- Windows now runs the whole test suite — `cargo test --workspace
  ...
  [§REQ-cross-platform.3](../requirements/cross-platform.md#3-tested-not-assumed) [§REQ-cross-platform.4](../requirements/cross-platform.md#4-portable-fixtures) [§AR-ci-release.1](../architecture/ci-release.spec.md#1-development-ci) (PR #97)
  ...
  their own. [§FS-rhei-programs.1.1](../functional-spec/rhei-programs.spec.md#11-string-form) **A create refused its own read**:
  ...
  first as before. [§FS-rhei-new.4](../functional-spec/rhei-new.spec.md#4-ids) **The supervisor's brief**: the sentence
  ...
  **`install-skills --link`**: symlinks are the one thing this command cannot do
  ...
  test pins. [§REQ-cross-platform.2](../requirements/cross-platform.md#2-parity) **The N-API cdylib**: its manifest says
  ...
  **A canonicalized path leaked its `\\?\` prefix** into the run report, the
  ...
  [§REQ-cross-platform.5](../requirements/cross-platform.md#5-paths-are-data) (PR #97)
...
- Windows CI now runs the test suite it can: every crate's unit tests,
  ...
  callback. [§AR-ci-release.1](../architecture/ci-release.spec.md#1-development-ci) (PR #94)
  ...
  [§FS-rhei-run-headless.3](../functional-spec/rhei-run-headless.spec.md#3-run-identity-and-liveness) **Rooted artifact paths**: a state declaring
  ...
  as absolute; both are rejected everywhere now. [§FS-rhei-states.1.3](../functional-spec/rhei-states.spec.md#13-validation-rules)
  **Snapshot lineage**: where the platform has no unprivileged symlinks the
  ...
  resolved nothing. [§FS-rhei-snapshots.7](../functional-spec/rhei-snapshots.spec.md#7-storage-layout) **Plan rewrites**: a file lock is
  ...
  depending on how it was linked. [§FS-rhei-distribution.1](../functional-spec/rhei-distribution.spec.md#1-release-targets) (PR #94)
  ...
  whether it is the same directory now. [§FS-rhei-init.5](../functional-spec/rhei-init.spec.md#5-discovery-report) (PR #94)
...
- CI now runs as two parallel jobs instead of one: `test` (fmt, clippy,
  ...
  [§AR-ci-release.1](../architecture/ci-release.spec.md#1-development-ci) (PR #93)
