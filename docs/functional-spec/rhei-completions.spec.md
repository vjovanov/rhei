...
|-------|-------|-------------------|---------------------|
| Bash | `bash` | `${XDG_DATA_HOME:-~/.local/share}/bash-completion/completions/<bin>` | `/usr/local/share/bash-completion/completions/<bin>` |
| Zsh | `zsh` | `~/.zfunc/_<bin>` | `/usr/local/share/zsh/site-functions/_<bin>` |
| Fish | `fish` | `${XDG_CONFIG_HOME:-~/.config}/fish/completions/<bin>.fish` | `/usr/local/share/fish/vendor_completions.d/<bin>.fish` |
| PowerShell | `powershell` | `${XDG_CONFIG_HOME:-~/.config}/powershell/<bin>-completions.ps1` | `/usr/local/share/powershell/Completions/<bin>-completions.ps1` |
| Elvish | `elvish` | `${XDG_CONFIG_HOME:-~/.config}/elvish/lib/<bin>-completions.elv` | `/usr/local/share/elvish/lib/<bin>-completions.elv` |
...
- The generated script is self-documenting: it starts with a short comment
  ...
  add it to for permanent use, and the `<bin> completions <shell> --install`
  alternative. Those commands name the invoked binary, so a script generated by
  `rh` tells the reader to rerun `rh`. All supported shells use `#` line
  comments, so the same header format applies everywhere.
- For Zsh the `#compdef <bin>` directive stays on the first line — `compinit`
...
- `rhei instantiate <TEMPLATE>` completes discovered project and user template names using the same precedence as `rhei templates` (§FS-rhei-templates.1): `<project>/.agent-grounds/rhei/templates/<name>/` first, then `~/.agent-grounds/rhei/templates/<name>/`, with the deprecated `.agents/rhei/templates/` searched after each. If the partially typed value is path-like (absolute, dot-prefixed, or contains `/`), directory completion is used instead.
...
- The current working directory and its `.agent-grounds/rhei/` tree, and the deprecated `.agents/rhei/` tree behind it (§FS-rhei-templates.1.1).
