...
- Per-state `supervise: task|state` to make a non-leaf task in that state a *supervisor* of its subtree, woken at every finished descendant (`task`) or every descendant transition (`state`) and holding the subtree in between (§FS-rhei-supervision)
...
| `supervise` | enum | No | Marks this state as a *supervising* state. A non-leaf task in it is woken at checkpoints of its subtree — `task`: after every descendant reaches a terminal state; `state`: after every descendant transition — and holds the subtree between visits. Requires a self-loop transition as the release edge. See [Supervision Specification](rhei-supervision.spec.md). |
