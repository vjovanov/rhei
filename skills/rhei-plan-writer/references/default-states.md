...
- `in-progress` — Actively implement the task and its child task nodes. Log progress per leaf node. When implementation is complete and self-tested, transition to `agent-review`.
- `agent-review` — A separate reviewing agent inspects the result against the task description, its child task nodes, and repository conventions. On pass, transition to `human-review` (or `completed` if no human gate is required). On fail, transition to `agent-review-fix` with concrete findings.
