    ...
    fn apply(&mut self, event: &RunEvent) {
        match event {
            ...
            RunEvent::RunLink { label, url } => {
                if !self.links.iter().any(|(_, existing)| existing == url) {
                    self.links.push((label.clone(), url.clone()));
                }
...
fn render_header(f: &mut ratatui::Frame, area: Rect, snapshot: &UiStateSnapshot) {
    ...
    let line = Line::from(vec![
        ...
        Span::raw(format!(
            ...
            snapshot.parallel, active, snapshot.total_tasks
        )),
        Span::raw(
            snapshot
                .links
        ...
        Span::raw(if snapshot.finished { "  [finished]" } else { "" }),
    ]);
