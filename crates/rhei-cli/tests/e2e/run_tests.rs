...
#[test]
fn run_callback_mode_stops_unbounded_review_fix_cycle() {
    let plan = r#"# Rhei: Review Cycle

## Tasks

### Task 1: Review me
**State:** agent-review-fix
"#;
    let machine = r#"name: review-cycle
version: 1
states:
  agent-review:
    description: Review
...
"#;
    ...
    let dir = unique_temp_dir("run-review-cycle");
    let plan_path = write_fixture_file(&dir, "plan.rhei.md", plan);
    let machine_path = write_fixture_file(&dir, "states.yaml", machine);

    let result = run_cli("run", &plan_path, &machine_path, &["--no-callbacks"]);
    assert_success(&result);
    assert_task_state(&plan_path, &machine_path, "1", "agent-review-fix");
    assert!(
        result.stdout.contains("already visited 'agent-review-fix'"),
        "run should report the repeated ready state instead of looping; got:\n{}",
        result.stdout
    );
    assert!(
        result.stdout.contains("2 transition(s) made"),
        "run should stop after one round trip; got:\n{}",
        result.stdout
    );
...
#[test]
fn run_dashboard_flag_prints_loopback_url_in_stdout_mode() {
    let (dir, plan_path, machine_path) = setup_single_file("run-dashboard-url", LINEAR_PLAN);

    let result = run_cli("run", &plan_path, &machine_path, &["--no-callbacks", "--dashboard"]);
    assert_success(&result);
    assert!(
        result.stdout.contains("Dashboard: http://127.0.0.1:"),
        "explicit dashboard should print the local URL; got:\n{}",
        result.stdout
    );
