...
// §FS-rhei-states.12: a machine composed with `extends: rhei` folds onto the
// built-in default and drives validation and transitions end to end.
#[test]
fn extends_machine_composes_onto_builtin_for_validate_and_transition() {
    let dir = unique_temp_dir("extends-machine");
    write_fixture_file(
        &dir,
        "states.yaml",
        r#"name: reviewed-rhei
extends: rhei
version: 1
states:
  in-review:
    description: Human review before completion
transitions:
  - from: pending
    to: in-review
  - from: in-review
...
profiles:
  default-rhei:
    initial: pending
    allowed: [pending, in-review, completed]
"#,
    );
    let plan = write_fixture_file(
        &dir,
        "plan.rhei.md",
        "# Rhei: Reviewed\n**States:** reviewed-rhei\n\n## Tasks\n\n### Task 1: Work\n**State:** pending\n",
    );
    ...
    assert!(
        ...
        "validate should accept the composed machine\nstdout: {}\nstderr: {}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );

    // The inherited (pending -> completed) group was removed, so the direct
    // transition is refused while the overlay's new pair works.
    let output = Command::new(env!("CARGO_BIN_EXE_rhei"))
        .arg("transition")
        .arg(&plan)
        .arg("--task")
    ...
    assert!(
        !output.status.success(),
        "removed inherited transition must be refused\nstdout: {}",
        String::from_utf8_lossy(&output.stdout)
    );
    ...
    assert!(
        output.status.success(),
        "overlay transition should succeed\nstdout: {}\nstderr: {}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    let rewritten = fs::read_to_string(&plan).expect("read plan");
    assert!(rewritten.contains("**State:** in-review"), "state advanced: {rewritten}");
