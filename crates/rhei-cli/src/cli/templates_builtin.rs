...
pub(super) fn materialize_builtin_template(name: &str) -> MietteResult<ExtractedTemplate> {
    let dir = BUILTIN_TEMPLATES
        .get_dir(name)
        .ok_or_else(|| miette!("no built-in template named '{name}'"))?;
    ...
    fs::create_dir_all(&root)
        .map_err(|err| miette!("failed to create '{}': {err}", root.display()))?;
    ...
    for file in files {
        let relative = file
            .path()
            .strip_prefix(name)
            .map_err(|_| miette!("built-in template '{name}' has an unexpected entry"))?;
